...
function findChairs(rooms, chairsNeed) {

    let spareChairs = [];

    for (let i = 0; i < rooms.length; i++) {
        const occupiedChairs = rooms[i][0];
        const totalChairs = rooms[i][1];
        const emptyChairs = totalChairs - occupiedChairs.length;
        
        if (emptyChairs >= chairsNeed) {
            spareChairs.push(chairsNeed);
            chairsNeed -= emptyChairs;
            break;
        } else {
            spareChairs.push(emptyChairs);
            chairsNeed -= emptyChairs;
        }
    }

    console.log(chairsNeed)

    if(chairsNeed > 0) return 0;
    else return spareChairs;
}

const rooms = [['XXX', 3], ['XXXXX', 6], ['XXXXXX', 9], ['XXX',2]];
const chairsNeed = 4;
const result = findChairs(rooms, chairsNeed);
